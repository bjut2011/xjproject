  ...
  def register
    if request.post?
       ...
       if params["stage"]
         ...
         config=Configinfo.find_by_name("hosts");
         url="202.100.170.37"
         if config
            url=config.value;
         end
         str="{\"result\":\"OK\",\"services\":{\"device_id\":\"#{dev_id}\",\"account\":\"\",\"active_date\":\"#{Time.now}\","
         str+="\"servers\": {\"portals\": [{\"hostname\": \"#{url}\",\"ssl_available\": \"no\",\"ssl_port\": \"443\", \"http_port\": \"80\",\"path\": \"\/api10\/\" }],"
         str+="\"auths\": [ {\"hostname\": \"#{url}\",\"ssl_available\": \"no\",\"ssl_port\":\"443\",\"http_port\": \"80\", \"path\": \"\/api10\/\"} ],"
         str +="\"platforms\": [ {\"hostname\": \"#{url}\", \"ssl_available\": \"no\", \"ssl_port\": \"443\", \"http_port\": \"80\",\"path\": \"\/api10\/\" }] } }}"
         respond_to do |format|
          format.json {render :json => {:result=>"OK", :message=>"OK",:services => {:device_id => dev_id,:account =>"",:active_date=>Time.now,:servers=>{:portals=>[{:hostname=>"#{url}",:ssl_available=>"no",:ssl_port=>"443",:http_port=>"80",:path=>"/api10/"}],:auths=>[{:hostname=>"#{url}",:ssl_available=>"no",:ssl_port=>"443",:http_port=>"80",:path=>"/api10/"}],:platforms=>[{:hostname=>"#{url}",:ssl_available=>"no",:ssl_port=>"443",:http_port=>"80",:path=>"/api10/"}]} } }}
