...
class WifidogController < ApplicationController
  ...
  def back_task(code)
    if code["task_code"]=="3000"
     ...
     respond_to do |format|
       format.json {render :json => {:task => {:task_code=>code["task_code"].to_i, :task_id=>code["task_id"],:task_params =>params },:result =>code["result"],:code =>code["code"],:message =>code["message"]}}
